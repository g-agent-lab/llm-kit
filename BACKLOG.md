...
|---|---|---|---|
| `typescript-nestjs` | **Done** | extracted из the NestJS host | NestJS 11 + Prisma 7 + Express 5 + SWC + ESLint flat + sonarjs + Jest + boundary plugin + dep-cruiser |
...
| Real-world brownfield bootstrap test (legacy non-NestJS-host) | **Done — 2026-05-13** | Loom brownfield 12/13 шагов pushed на `loom/master`. Baselines 194/12/0. Step 13 (first ralphex iteration) deferred до отдельной сессии. |
...
| Operator profile separation (operator-specific mode vs universal core) | **Rejected by design** — see шапка `UNIVERSAL_CORE.md` «Operator profile». Premature abstraction под гипотетический use case. | n/a |

## Real-world findings — loom brownfield bootstrap (v1.3)

**Major milestone:** первое полное brownfield bootstrap on non-NestJS-host проекте. 12/13 шагов завершены, push'нуто на `loom/master`. Step 13 (first ralphex iteration) defer'ен до отдельной сессии. Это закрывает **Real-world brownfield validation** white-space из validation backlog.
